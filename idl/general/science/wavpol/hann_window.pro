...
;
;EXAMPLE: hann_window(7,/rescale)
;
;CALLING SEQUENCE: hann_window(m, /rescale)
;
;INPUTS:
;       m: length of desired smoothing window
;;
;Keywords:
;  rescale (optional):  If set, divide by the sum of the w(n) values so the weights sum to 1.0
;
...
function hann_window, m, rescale=rescale
   w = 0.5 - 0.5*cos(2.0D * !dpi * dindgen(m)/(m - 1.0D))
   tot = total(w)
   if keyword_set(rescale) then return, w/tot else return, w
   return, w
end