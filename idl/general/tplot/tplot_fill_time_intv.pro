...
;         as a structure in the fill_time_intv structure and a polyopt
;         structure,
...
;
;set polyopt:{line_fill:1,orientation:45} to use angled 
...
;two times and a color value, or two X n times and n colors,
  If(tag_exist(newlim,'fill_time_intv')) Then Begin
     ...
     If(tag_exist(newlim.fill_time_intv, 'polyopt')) Then Begin
        pollyo = 1b
;allow for different options for different intervals
        If(n_elements(newlim.fill_time_intv.polyopt) Ne n_intv) Then Begin
           p_intv = replicate(newlim.fill_time_intv.polyopt[0], n_intv)
        Endif Else p_intv = newlim.fill_time_intv.polyopt
     Endif Else pollyo = 0b
     For j = 0, n_intv-1 Do Begin
        ...
        IF(pollyo) Then Begin ;special case, set line_fill=255 for a solid color
           If(tag_exist(p_intv[j], 'line_fill') && p_intv[j].line_fill Eq 255) Then Begin
              polyfill, x_rct, y_rct, color = c_intv[j]
           Endif Else polyfill, x_rct, y_rct, color = c_intv[j], _extra = p_intv[j]
        Endif Else polyfill, x_rct, y_rct, color = c_intv[j]
     Endfor
