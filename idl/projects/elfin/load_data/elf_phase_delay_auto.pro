...
;
      ...
      ; :Author: lauraigs
      ;-
pro elf_phase_delay_auto, pick_times=pick_times, new_config=new_config, probe=probe, Echannels=Echannels, overwrite_attitude=overwrite_attitude, check_nonmonotonic=check_nonmonotonic, sstart = sstart, send = send, soldtimes = soldtimes

  ;elf_init
  ;
  tplot_options, 'xmargin', [15,9]
  cwdirname='/Users/lauraigs/Documents/Research/ELFIN/'
  cwd,cwdirname
  ...
  file='el'+probe+'_epde_phase_delays_copy.csv'
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  ...
  plotxy,[[reform(pa2plots_even[0,*])],[reform(pef2plots_even[0,*])]], $
    ...
    ' UT'+', dSectr2add = '+strtrim(string(dSectr2add,format="(I7)"),1)+', dPhAng2add = '+strtrim(string(dPhAng2add,format="(f5.1)"),1)+'deg, dPA290_est='+string(deltaPA_est,format="(f5.1)")+'deg',xtitle='PA [deg]',ytitle='Normalized Flux'
  plotxy,[[reform(pa2plots_odd[0,*])],[reform(pef2plots_odd[0,*])]],/over,psym=-1,colors=['b'],title=' ',xtitle='PA [deg]',ytitle='Normalized Flux'
  for jthspec=0,n_elements(ispeceven)-1 do plotxy,[[reform(pa2plots_even[jthspec,*])],[reform(pef2plots_even[jthspec,*])]],/over,psym=-1,colors=['r'],title=' ',xtitle='PA [deg]',ytitle='Normalized Flux'
  for jthspec=0,n_elements(ispeceven)-1 do plotxy,[[pafitminus90_even[jthspec]+90.,pafitminus90_even[jthspec]+90.],[0.2*maxy,maxy]],/over,colors=['r'],title=' ',xtitle='PA [deg]',ytitle='Normalized Flux'
  for jthspec=0,n_elements(ispecodd)-1 do plotxy,[[reform(pa2plots_odd[jthspec,*])],[reform(pef2plots_odd[jthspec,*])]],/over,psym=-1,colors=['b'],title=' ',xtitle='PA [deg]',ytitle='Normalized Flux'
  for jthspec=0,n_elements(ispecodd)-1 do plotxy,[[pafitminus90_odd[jthspec]+90.,pafitminus90_odd[jthspec]+90.],[0.2*maxy,maxy]],/over,colors=['b'],title=' ',xtitle='PA [deg]',ytitle='Normalized Flux'
  plotxy,[[xfit],[yfit_odd]],/over,colors=['o'],title=' ',linestyle=2,thick=5,yrange=[miny,maxy],xtitle='PA [deg]',ytitle='Normalized Flux'
  plotxy,[[xfit],[yfit_even]],/over,colors=['g'],title=' ',linestyle=2,thick=5,yrange=[miny,maxy],xtitle='PA [deg]',ytitle='Normalized Flux'
  ;
  ...
  deltaPA_est = round(deltaPA_est*10)/10.
  
  maxiter = 10
  if deltaPA_est eq 0 or iters gt maxiter then begin
    if iters gt 10 then print, 'Max iterations reached; using latest result'
  ...
  if abs(dSectr2add) gt 4 then begin
  ...
  newentry = [time_double(tstart), time_double(tend), dSectr2add, dPhang2add, LatestMedianSectr, LatestMedianPhAng, badFlag]
  ...
  if time_double(tstart) ge starttimes[newindex]-60.*5. and time_double(tstart) le starttimes[newindex]+60.*5. then begin
    replace = 1
    tbr = where(time_double(tstart) ge starttimes[newindex]-60.*5. and time_double(tstart) le starttimes[newindex]+60.*5.)
