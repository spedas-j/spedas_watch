...
function swx_spc_met_to_unixtime_old,met,reverse=reverse
...
;
;+
;Function:  swx_spc_met_to_unixtime
;Purpose:  Convert MET (mission Elapsed Time) to Unix Time  (which is almost equivalent to UTC)
;see also:  "swx_spc_unixtime_to_met" for the reverse conversion
...
function swx_spc_met_to_unixtime,input,reverse=reverse,correct_clockdrift=correct_clockdrift,reset=reset,ephemeris_time=et,kernels=kernels  ;,prelaunch = prelaunch
  ...
  common swx_spc_met_to_unixtime_com, cor_clkdrift, icy_installed, kernel_verified, time_verified, sclk, tls
  ...
  if keyword_set(cor_clkdrift) then begin
    if  n_elements(kernel_verified) eq 0 || keyword_set(reset) then begin ; check for cspice first
      if spice_test() then begin
        ...
        tls  = swx_spice_kernels('LSK',/load,trange=[0,1])  ; getting only the LSK file
        sclk = swx_spice_kernels('SCK',/load,trange=[0,1]) ;dummy trange is set to prevent time prompt if timespan not set
