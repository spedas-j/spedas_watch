...
FUNCTION swx_gen_apdat::Init,apid,name,_EXTRA=ex
  COMPILE_OPT IDL2
  ...
  self.data = dynamicarray(name=self.name,tplot_tagnames = self.ttags)
  RETURN, 1
END


PRO swx_gen_apdat::Clear, tplot_names=tplot_names
  COMPILE_OPT IDL2
...
function swx_gen_apdat::info,header=header
...
PRO swx_gen_apdat::print,dlevel=dlevel,verbose=verbose,strng,header=header
  print,self.info(header=header)
END


;pro swx_gen_apdat::store_data,  strct,pname,verbose=verbose
...
pro swx_gen_apdat::increment_counters,ccsds,source_dict=source_dict
  self.npkts += 1
...
function swx_gen_apdat::decom_aggregate,str=str,ccsds0,source_dict=source_dict

  n = ccsds0.aggregate

  if n ne 0 then begin
    buffer  = swx_ccsds_data(ccsds0)
    ccsds = ccsds0
    ccsds.aggregate =0
...
function swx_gen_apdat::decom,ccsds,source_dict=source_dict   ;header
...
pro swx_gen_apdat::handler,ccsds,source_dict=source_dict ;,header,source_info=source_info
...
pro swx_gen_apdat::create_tplot_vars,ttags=ttags
...
pro swx_gen_apdat::finish,ttags=ttags
...
function swx_gen_apdat::sw_version
  ...
  sw_hash['svn_changedate'] = '$LastChangedDate: 2023-12-02 03:15:48 -0800 (Sat, 02 Dec 2023) $'
  sw_hash['svn_revision '] = '$LastChangedRevision: 32265 $'

  return,sw_hash
end

function swx_gen_apdat::cdf_global_attributes
  ...
  ;  global_att['SVN_CHANGEDATE'] = '$LastChangedDate: 2023-12-02 03:15:48 -0800 (Sat, 02 Dec 2023) $'
  ;  global_att['SVN_REVISION'] = '$LastChangedRevision: 32265 $'

  return,global_att
end


;pro swx_gen_apdat::cdf_create_data_vars, fileid, var, vattributes=atts, varstr
;
;message,'Obsolete'
...
;      val = array.(i)
;      swx_swp_cdf_var_att_create,fileid,varnames[i],val,attributes=atts
;    endfor
;  endif
;
;end
...
function swx_gen_apdat::cdf_makeobj,  datavary, datanovary,  vnames=vnames, ignore=ignore,global_att=global_att,_extra=ex
...
pro swx_gen_apdat::cdf_makefile,trange=trange,verbose=verbose,filename=filename,parents=parents
...
pro swx_gen_apdat::sav_makefile,sav_file=sav_file,parents=parents,verbose=verbose
  ...
  for i=0,ndays do begin
    ...
    self.data.name=self.name ;in case swx_swp_apdat_init updated the object name (e.g., from wrp_P5 to wrp_P5P7)
...
;
;pro swx_gen_apdat::cdf_create_file,cdftags=cdftags,trange=trange   ; this routine is almost obsolete
...
;  pathname =  swx_file_retrieve(self.cdf_pathname ,trange=trange,/create_dir,/daily_names)
...
PRO swx_gen_apdat::GetProperty,data=data, array=array, npkts=npkts,lost_pkts=lost_pkts, apid=apid, name=name,  typename=typename, $
...
PRO swx_gen_apdat::SetProperty,apid=apid, _extra=ex
  COMPILE_OPT IDL2
