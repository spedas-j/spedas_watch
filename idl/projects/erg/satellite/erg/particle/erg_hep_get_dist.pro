...
function erg_hep_get_dist $
  ...
  if ~strcmp(name, 'erg_hep_l2_FEDU_L') $
     and ~strcmp(name, 'erg_hep_l2_FEDU_H') then begin
    dprint, 'Variable '+name+' is not acceptable. This routine can take only erg_hep_l2_FEDU_? as the argument.'
    return, 0
  endif
  ...
  t_fedu =  d.x & fedu = d.y & undefine, d
  ...
  id = where( strcmp( cdfi.vars.name, dtype+'_'+suf+'_Energy' ) )
  ...
  id = where( strcmp( cdfi.vars.name, dtype+'_'+suf+'_Angle_sga' ) )
  angarr = *( cdfi.vars[id].dataptr )  ;;[elev/phi, (azm)] in SGA

  ;; converted to the flux dirs
  sphere_to_cart, replicate(1., n_elements(angarr[0, *])), reform(angarr[0, *]), reform(angarr[1, *]), $
                  ex, ey, ez
  ...
  phissi = reform( angarr[1, *] ) - (90.+21.6) ;; [(azm)] 
