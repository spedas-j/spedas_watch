  ...
  for i=0L,nd-1 do begin ;loop over days
    ...
    if keyword_set(make_cdf) then begin
      if 1 then begin  ; make cdf files
        cdf_pathformat = output_prefix+'cdf/YYYY/MM/DD/spp_$NAME$_L1_YYYYMMDD.cdf'
        spp_apdat_info,'*_hkp',cdf_pathname = cdf_pathformat
        spp_apdat_info,'*_tof',cdf_pathname = cdf_pathformat
