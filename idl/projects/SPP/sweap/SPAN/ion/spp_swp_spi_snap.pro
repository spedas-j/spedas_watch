...
; Ali 20190601
; $LastChangedBy: ali $
; $LastChangedDate: 2019-06-14 14:03:52 -0700 (Fri, 14 Jun 2019) $
; $LastChangedRevision: 27350 $
...
pro spp_swp_spi_snap,level=level,types=types,trange=trange,load=load,all=all,maxcalc=maxcalc,nmaxcalc=nmaxcalc,rate=rate,accum=accum,snap=snap,spectra=spectra
  ...
  if level eq 'L1' then begin
    ...
    foreach type,types do begin
      ...
      dim=size(/dimen,data)
      nt=dim[1]
      if keyword_set(accum) then store_data,prefix+'tot_accum_period',vardata.time,vardata.tot_accum_period,dlim={ylog:1,ystyle:3}
      if keyword_set(maxcalc) then begin
        maxcounts=max(data,dim=1)
        store_data,prefix+'maxcounts',vardata.time,maxcounts,dlim={ylog:1,yrange:[1,1e5],ystyle:3,constant:[1,2.^16]}
      endif
      if keyword_set(nmaxcalc) then begin
        ...
        store_data,prefix+'maxbins',vardata.time,nmax,dlim={ylog:1,ystyle:3}
      endif
      if keyword_set(rate) then begin
        data/=transpose(rebin(vardata.tot_accum_period,[nt,dim[0]]))
        countrate='Rate'
        ...
        '0':begin
          if keyword_set(snap) then begin
            ...
            p=image(transpose(alog10(data_theta)),.5+findgen(8),.5+findgen(32),rgb=colortable(33),min=min,max=max,axis_style=axis_style,$
              title=type,xtitle='Anode #',ytitle='Energy bin',xrange=[0,8],yrange=[33,0],/current,layout=[5,2,1+5*mbin])
            p=image(alog10(data_phi),.5+findgen(8),.5+findgen(32),rgb=colortable(33),min=min,max=max,axis_style=axis_style,$
              title=type,xtitle='Deflection bin',ytitle='Energy bin',xrange=[0,9],yrange=[33,0],/current,layout=[5,2,2+5*mbin])
            p=image(transpose(alog10(data_energy)),0.5+findgen(8),0.5+findgen(8),rgb=colortable(33),min=min,max=max,axis_style=axis_style,$
              title=type,ytitle='Deflection bin',xtitle='Anode #',yrange=[0,9],xrange=[0,8],/current,layout=[5,5,4+mbin])
          endif
        ...
        '1':begin
          if keyword_set(snap) then begin
            p=image(alog10(data),.5+findgen(8),.5+findgen(32),rgb=colortable(33),min=min,max=max,axis_style=axis_style,$
              title=type,xtitle='Deflection bin',ytitle='Energy bin',xrange=[0,9],yrange=[33,0],/current,layout=[5,2,3+5*mbin])
          endif
          if keyword_set(spectra) then begin
        ...
        '2':begin
          if keyword_set(snap) then begin
            p=image(alog10(data),.5+findgen(32),.5+findgen(16),rgb=colortable(33),min=min,max=max,axis_style=axis_style,$
              title=type,xtitle='Energy bin',ytitle='Mass bin',xrange=[33,0],yrange=[0,17],/current,layout=[2,5,10-2*mbin])
          endif
          if keyword_set(spectra) then begin
    ...
    if keyword_set(snap) then begin
      p=colorbar(title='Log10 '+countrate,/orientation,position=[.95,.2,.99,.7])
