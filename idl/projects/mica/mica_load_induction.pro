...
;         The no download feature is not implemented yet
...
pro mica_load_induction, $
  ...
  for i=0,n_elements(site_code)-1 do begin
    ...
    if filenames[0] NE '' or no_download NE 0 then local_test=file_test(filenames)
    ...
    if(total(local_test) ge 1) then begin  
      ...
      cdf2tplot, file=filenames, verbose=source.verbose, $
        suffix='_'+site_code[i], tplotnames=tplotnames

      get_data, 'spectra_x_1Hz_' + site_code[i], data = spec_tmp
      w_fill = where(spec_tmp.y gt 1000., count)
      if count gt 0 then spec_tmp.y[w_fill] = !values.f_nan
      store_data, 'spectra_x_1Hz_' + site_code[i], data = spec_tmp

      get_data, 'spectra_y_1Hz_' + site_code[i], data = spec_tmp
      w_fill = where(spec_tmp.y gt 1000., count)
      if count gt 0 then spec_tmp.y[w_fill] = !values.f_nan
      store_data, 'spectra_y_1Hz_' + site_code[i], data = spec_tmp

      get_data, 'spectra_x_5Hz_' + site_code[i], data = spec_tmp
      w_fill = where(spec_tmp.y gt 1000., count)
      if count gt 0 then spec_tmp.y[w_fill] = !values.f_nan
      store_data, 'spectra_x_5Hz_' + site_code[i], data = spec_tmp

      get_data, 'spectra_y_5Hz_' + site_code[i], data = spec_tmp
      w_fill = where(spec_tmp.y gt 1000., count)
      if count gt 0 then spec_tmp.y[w_fill] = !values.f_nan
      store_data, 'spectra_y_5Hz_' + site_code[i], data = spec_tmp
