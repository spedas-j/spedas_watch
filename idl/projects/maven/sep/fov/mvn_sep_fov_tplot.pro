...
pro mvn_sep_fov_tplot,tplot=tplot,store=store,fraction=fraction,resdeg=resdeg
  ...
  if keyword_set(store) then begin
    ...
    store_data,'mvn_sep_sun_infov',times,transpose([pos[0,*].sun gt cos(!dtor*25.),pos[2,*].sun gt cos(!dtor*25.),-pos[0,*].sun gt cos(!dtor*25.),-pos[2,*].sun gt cos(!dtor*25.),.1+transpose(pdm.sun gt pdm.mar)]),dlim={colors:'brcmg',labels:['1F','2F','1R','2R','Shadow'],ystyle:3,labflag:-1,panel_size:.3}
    store_data,'mvn_sep_sx1_infov',times,transpose([pos[0,*].sx1 gt cos(!dtor*15.),pos[2,*].sx1 gt cos(!dtor*15.),-pos[0,*].sx1 gt cos(!dtor*15.),-pos[2,*].sx1 gt cos(!dtor*15.),.1+transpose(pdm.sx1 gt pdm.mar)]),dlim={colors:'brcmg',labels:['1F','2F','1R','2R','Shadow'],ystyle:3,labflag:-1,panel_size:.3}
    store_data,'mvn_sep_att',times,transpose(mvn_sep_fov.att)#[[1,0],[0,1.1]],dlim={colors:'br',labels:['SEP1','SEP2'],ystyle:3,labflag:-1,panel_size:.3}
