...
pro swfo_stis_tplot,name,add=add,setlim=setlim
  ...
  if keyword_set(setlim) then begin
    ...
    options,'*nse_NHIST',spec=1,panel_size=2,/no_interp,/zlog,zrange=[10,4000.],constant=findgen(6)*10+5
    options,'*sci_COUNTS',spec=1,panel_size=8,ystyle=3,/no_interp,/zlog,zrange=[1,4000.],constant=findgen(15)*48
    options,'*hkp?_ADC_*',constant=0.
    channels=['CH1','CH2','CH3','CH4','CH5','CH6']
    options,'*hkp1_RATES_COUNTER',panel_size=2,/ylog,yrange=[.5,1e5],ystyle=3,colors='bgrmcd',psym=-1,symsize=.5,labels=channels,labflag=-1
    options,'*hkp1_ADC_BASELINES',panel_size=2,colors='bgrmcd',labels=channels,labflag=-1
    options,'*hkp1_ADC_VOLTAGES',panel_size=2,colors='bgrmc',labels=['1.5vd','3.3vd','5vd','-5va','+5va'],labflag=-1
    options,'*hkp1_ADC_TEMPS',colors='bgr',labels=['DAP','S1','S2'],labflag=-1
    ...
    options,'*hkp1_DAC_VALUES',panel_size=2,ystyle=3,yrange=[0,0xffff],colors='bgrmmcdcbgrk',labels=dacs,labflag=-1
    options,'*hkp2_DAC_VALUES',panel_size=2,ystyle=3,yrange=[0,0xff  ],colors='bgrmmcdcbgrk',labels=dacs,labflag=-1
    ...
    store_data,'swfo_stis_hkp1_RATES_FREQ',data='swfo_stis_hkp1_'+['RATES_COUNTER','PULSER_FREQUENCY'],dlim={panel_size:2,yrange:[.5,1e5]}
  ...
  case plot_name of
    'SUM': tplot,add=add,'*hkp?_DAC_* *hkp1_RATES_FREQ *sci_COUNTS *NHIST *hkp1_*REMAIN* *hkp1*BITS *hkp1_ADC_*'; *hkp1_ADC_*'
