...
; $URL: svn+ssh://thmsvn@ambrosia.ssl.berkeley.edu:36867/repos/spdsoft/trunk/projects/SWFO/STIS/generic_apdat__define.pro $
;-
;COMPILE_OPT IDL2


FUNCTION generic_apdat::Init,apid,name,_EXTRA=ex,verbose=verbose  ;,mission=mission
  COMPILE_OPT IDL2
...
;  if isa(instrument,/string) THEN self.instrument = instrument
  self.apid  =apid
  self.dlevel = 2
  ...
  self.ncdf_directory = root_data_dir() +  self.mission+'/data/sci/'+self.instrument+'/prelaunch/realtime/test/'
...
function generic_apdat::decom,ccsds,source_dict=source_dict   ; general purpose - should be overloaded - does nothing special here
...
function generic_apdat::decom_time,ccsds,source_dict=source_dict
...
pro generic_apdat::create_tplot_vars,ttags=ttags,verbose=verbose
  ...
  if isa(dyndata,'dynamicarray') && keyword_set(self.tname) then begin
    store_data,self.tname,data=dyndata, tagnames=ttags, gap_tag='GAP',verbose=0,/silent   ;,verbose = self.verbose ;,/silent
  endif
end
...
pro generic_apdat::sav_makefile,sav_format=sav_format,parent=parent,verbose=verbose
...
function generic_apdat::get_ncdf_master_structure,template_name
...
pro generic_apdat::ncdf_make_file,ddata=ddata,pathformat=pathformat,testdir=testdir,ret_filename=ret_filename,type=type,trange=trange,resolution=resolution
