...
; make an image for just this swath
        ...
        if color_table [k] ne 12 then loadct, color_table[k] else  generate_custom_color_table, 12
        ...
        If keyword_set (satellite) then begin
           ...
           map_set, 0, 180.0,/cylindrical,/grid,/isotropic, $
                    xmargin = 0, ymargin = 0
        endif
...
; ad magnetic field contours
        ...
        if color_table [k] ne 12 then loadct, $
           color_table[k] else  generate_custom_color_table, 12
...
; record the spacecraft local time on the image
       ...
       if keyword_set (jpeg) then make_JPEG,output_directory + projection + '_' +$
...
; We've made individual swaths.  Now tie them all together.
  ...
  if swath_count gt 1 then begin
