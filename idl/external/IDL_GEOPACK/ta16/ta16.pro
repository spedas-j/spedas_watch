...
function ta16,tarray,rgsm_array,pdyn,yimf,symc,xind, $
  ...
  symc_size = size(symc, /dimensions)
  ...
  if n_elements(symc_size) ne 1 then begin
    message, /continue, 'symc has incorrect dimensions'
    return, -1L
  endif
  ...
  if symc_size[0] eq 0 then begin
    symc_array = replicate(symc,t_size)
  endif else if t_size[0] ne symc_size[0] then begin
    message, /continue, 'number of times in tarray does not match number of elements in symc_array'
    return, -1L
  endif else symc_array = symc
  ...
  while i lt nperiod do begin
    ...
    if idx[0] ne -1L then begin
      ...
      out_array[idx, 2] = igrf_bz + ta16_bz
