pro tomni2bindex, imf_by=imf_by,imf_bz=imf_bz, sw_prot_dens=sw_prot_dens, sw_speed=sw_speed,out_name=out_name
   ...
   if ~keyword_set(sw_prot_dens) || (size(sw_prot_dens,/type) ne 7) then begin
     dprint,'Required sw_prot_dens parameter missing or invalid'
     return
   endif
   ...
   get_data,sw_prot_dens,data=sw_prot_dens_d
   ...
   if size(sw_prot_dens_d,/type) ne 8 then begin
     dprint,sw_prot_dens+' is not a valid tplot variable name.'
     return
   endif
   ...
   b_index = omni2bindex(imf_by=imf_by_d.y, imf_bz=imf_bz_d.y, sw_prot_dens=sw_prot_dens_d.y, sw_speed=sw_speed_d.y)   

   store_data,out_name,data={x:imf_by_d.x, y:b_index}
end
